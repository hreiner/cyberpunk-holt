...
/**
 * Passe de simplification : la signalétique décorative (chevrons de sortie, gyrophares) répétée
 * dans chaque salle ne disait rien de plus que la balise d'objectif (voir
 * `ExploreView.setObjectiveTarget`) et lui faisait concurrence -- quatre signaux rouges/bleus
 * à l'écran, dont un seul comptait. Il ne reste que le gyrophare de la salle 3, que le gaz
 * justifie. Même logique pour le mobilier : voir la note en tête de `src/data/maps/centre-examen.ts`.
 */
