...
export function createExploreNpcRig(entityId: string, seated = false): ExploreNpcRig {
  return entityId === 'salle1.chien' ? new DogNpcRig(entityId) : new HumanNpcRig(entityId, seated);
}
...
class HumanNpcRig implements ExploreNpcRig {
  ...
  constructor(entityId: string, seated: boolean) {
    ...
    if (seated) this.rig.playExplorationPose('sit');
  }
