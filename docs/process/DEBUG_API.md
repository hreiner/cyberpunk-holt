...
| `node().recall` | `PresentedLine[]`, ou absent | répliques d'un noeud précédent rappelées en tête de celui-ci (`DialogueNode.recall`, voir 07-DIALOGUE-FORMAT.md) : à l'examen écrit, l'énoncé du surveillant reste sous les yeux du joueur une fois la triche jouée et la question quittée. Absent sur l'immense majorité des noeuds. |
...
| `exploreRenderStats()` | `{ drawCalls, triangles, geometries, textures, gpu, pixelRatio }`, ou `null` | compteurs Three/WebGL de la dernière image d'exploration, pour mesurer L7 sur un matériel et un parcours documentés ; hors exploration, renvoie `null`. `gpu` est le nom que le pilote rapporte (`null` si le navigateur masque l'extension) et `pixelRatio` le rapport de pixels effectif : c'est avec ces deux-là qu'on distingue « le jeu est trop lourd » de « ce navigateur-ci rend sur la carte intégrée » — deux navigateurs sur la même machine ne choisissent pas forcément le même GPU |
